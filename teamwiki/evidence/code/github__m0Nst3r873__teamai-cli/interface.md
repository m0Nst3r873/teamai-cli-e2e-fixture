---
title: github__m0Nst3r873__teamai-cli interface
...
- `UserStats` ← src/types.ts:329 [EXTRACTED]
  ...
  ```
- `SessionRecord` ← src/types.ts:337 [EXTRACTED]
  ```
  export interface SessionRecord {
  ```
- `DashboardSessionStatus` ← src/types.ts:364 [EXTRACTED]
  ```
  export type DashboardSessionStatus = 'running' | 'waiting_for_input' | 'error' | 'idle' | 'stopped';
  ```
- `DashboardEventType` ← src/types.ts:366 [EXTRACTED]
  ```
  export type DashboardEventType = 'session_start' | 'tool_use' | 'prompt_submit' | 'stop' | 'process_exit';
  ```
- `DashboardEvent` ← src/types.ts:368 [EXTRACTED]
  ```
  export interface DashboardEvent {
  ```
- `DashboardSession` ← src/types.ts:393 [EXTRACTED]
  ```
  export interface DashboardSession {
  ```
- `ContributeState` ← src/types.ts:449 [EXTRACTED]
  ```
  export interface ContributeState {
  ```
- `LearningDocMeta` ← src/types.ts:515 [EXTRACTED]
  ```
  export interface LearningDocMeta {
  ```
- `KnowledgeType` ← src/types.ts:523 [EXTRACTED]
  ```
  export type KnowledgeType = 'learnings' | 'docs' | 'rules' | 'skills';
  ```
- `KnowledgeDomain` ← src/types.ts:534 [EXTRACTED]
  ```
  export type KnowledgeDomain = 'technical' | 'ops' | 'support' | 'neutral';
  ```
- `SearchIndexEntry` ← src/types.ts:537 [EXTRACTED]
  ```
  export interface SearchIndexEntry {
  ```
- `SearchIndex` ← src/types.ts:566 [EXTRACTED]
  ```
  export interface SearchIndex {
  ```
- `UserVotes` ← src/types.ts:582 [EXTRACTED]
  ```
  export interface UserVotes {
  ```
- `CultureFrontmatter` ← src/types.ts:605 [EXTRACTED]
  ```
  export type CultureFrontmatter = z.infer<typeof CultureFrontmatterSchema>;
  ```
- `MRData` ← src/types.ts:689 [EXTRACTED]
  ```
  export interface MRData {
  ```
- `ClassifiedItem` ← src/types.ts:709 [EXTRACTED]
  ```
  export interface ClassifiedItem {
  ```
- `LearningDraft` ← src/types.ts:731 [EXTRACTED]
  ```
  export interface LearningDraft {
  ```
- `CodebaseSuggestion` ← src/types.ts:743 [EXTRACTED]
  ```
  export interface CodebaseSuggestion {
  ```
- `ImportSessionItem` ← src/types.ts:781 [EXTRACTED]
  ```
  export interface ImportSessionItem {
  ```
- `ImportSession` ← src/types.ts:799 [EXTRACTED]
  ```
